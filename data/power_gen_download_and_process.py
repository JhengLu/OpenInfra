...
def prepareSaveEIAData(EIA_data_path, save_path):
    global eba_json
    global ba_list
    global ts_list

    # Define paths for each output
    eba_json_path = os.path.join(save_path, 'eba_json.json')
    ba_list_path = os.path.join(save_path, 'ba_list.txt')
    ts_list_path = os.path.join(save_path, 'ts_list.txt')
    ...
    # Save eba_json as JSON
    eba_json.to_json(eba_json_path, orient='records', lines=True)

    # Save ba_list and ts_list as text files
    with open(ba_list_path, 'w') as f:
        json.dump(ba_list, f)
...
def loadPreparedEIAData(save_path):
    # Define paths for each output
    eba_json_path = os.path.join(save_path, 'eba_json.json')
    ba_list_path = os.path.join(save_path, 'ba_list.txt')
    ts_list_path = os.path.join(save_path, 'ts_list.txt')
    ...
    # Load eba_json
    eba_json = pd.read_json(eba_json_path, orient='records', lines=True)

    # Load ba_list and ts_list from text files
    with open(ba_list_path, 'r') as f:
        ba_list = json.load(f)
    
    with open(ts_list_path, 'r') as f:
        ts_list = json.load(f)
