...
class Chiller:
    ...
    def simulate_chiller_power_usage(self, server_temperature, energy_grid_temperature):
        ...
        datacenter_cooling_energy = (
                                                self.number_of_servers * self.flow_rate * self.water_density * self.water_specific_heat_capacity * server_temp_diff) / self.COP
        energy_grid_cooling_energy = (
                                                 self.flow_rate * self.water_density * self.water_specific_heat_capacity * energy_grid_temp_diff) / self.COP
...
class Pump:
    ...
    def simulate_pump_power_usage(self, number_of_chillers):
        return (
                    number_of_chillers * self.gravity * self.flow_rate * self.fluid_density * self.pressure_head) / self.pump_efficiency